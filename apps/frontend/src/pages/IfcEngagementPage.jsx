...
export default function IfcEngagementPage() {
  ...
  return (
    <PageShell maxWidth="max-w-[1800px]">
      <Link
        to={`/app/audit-planning/engagements/${encodeURIComponent(eid)}?tab=ifc`}
        className="inline-flex items-center gap-2 text-xs font-mono uppercase text-[#737373] hover:text-white mb-4"
      >
        <ArrowLeft size={14} /> Engagement hub (IFC tab)
      </Link>
      <IfcEvaluationPanel engagementId={eid} compact={false} />
    </PageShell>
  );
}
