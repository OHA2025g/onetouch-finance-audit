...
async def copilot_ask(body: CopilotAskRequest, current=Depends(get_current_user)):
    out = await ask_copilot(
        db, body.question, current["email"], body.session_id, mode=body.mode,
    )
...
async def copilot_generate_cfo_summary(body: Dict[str, Any], current=Depends(get_current_user)):
    ...
    out = await ask_copilot(db, q, current["email"], body.get("session_id"), mode="cfo")
...
async def copilot_generate_audit_procedure(body: Dict[str, Any], current=Depends(get_current_user)):
    ...
    out = await ask_copilot(db, q, current["email"], body.get("session_id"), mode=_copilot_mode_for_role(current.get("role") or "auditor"))
...
async def evidence_intelligence_summary(current=Depends(get_current_user)):
